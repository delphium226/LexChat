...
| `server_py/src/agent/tools/` | Tools package (split from the former single-file tools.py; `__init__.py` re-exports the full surface): `schemas.py` (tool schemas, `get_manager_tools`, `get_worker_tools`), `lex.py` (`_slim_search_results`, jurisdiction matching), `parliament.py` (`execute_parliament_tool`, `_slim_hansard_results`, `_search_committee_transcripts_db`, SP HTML parsers), `caselaw.py` (Atom/AKN parsing), `executor.py` (`execute_worker_tool`) |
