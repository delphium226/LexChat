...
| `server_py/src/agent/agent_core.py` | Provider-agnostic agent core: `run_worker_agent`, Deep Research `draft_research_plan` (Phase A planner) + `run_deep_research` (Phase B code-orchestrated execution + synthesis), `_normalise_plan`, `_build_step_brief` |
...
- **Feature flags** — `prompt_caching_enabled` and `tool_memo_enabled` (both default ON = current behaviour), consumed via the request provider-config ContextVar; gate the D5 provider prompt caching and tool memo respectively. An absent key = enabled, so direct callers/tests/parliament bot are unchanged.
