      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-paper p-6 rounded-lg shadow">
          ...
          <p className="text-xs text-ink-500 mb-4">
            Each call = one round-trip to the AI model. More calls = the agent performed more research tool loops.
          </p>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={llmDistribution} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  <XAxis
                    ...
                    stroke="#9ca3af"
                    tick={{ fontSize: 11 }}
                    label={{ value: 'LLM calls', position: 'insideBottom', offset: -2, fontSize: 10, fill: '#9ca3af' }}
                  />
              ...
              {[
                {
                  ...
                  tooltip:
                    'Cumulative time waiting for mistral-large (via Ollama) to generate tokens, summed across all calls in the query. Each call sends the full prompt — conversation history, system instructions, and any tool results — and waits for a complete response. Generation time scales with input context length and output length. This is almost always the largest component on GPU-constrained hardware. A sudden increase here typically means the model has fallen back to CPU, or concurrent requests are queuing behind an ongoing generation.',
                  ms: kpi.avgLlmMs,
                  color: PERF_COLORS.llm,
                },
                {
                  label: 'Legal Database Lookups',
                  tooltip:
                    "Total time spent making HTTP requests to the LEX API — the UK government's database of legislation, statutory instruments, and court judgments — summed across all lookups in the query. Each request involves a network round-trip to the LEX server, query execution, and transmission of document payloads back to AILA. On the air-gapped internal network, high latency here usually points to LEX server load or large result sets rather than connectivity issues.",
                  ms: kpi.avgLexMs,
                  color: PERF_COLORS.lex,
                },
                {
                  label: 'Request Queue Wait',
                  tooltip:
                    "Time the query spent waiting in the server's internal queue before the agent pipeline began. AILA serialises AI model access to avoid GPU memory contention — if a query is already running, new ones queue behind it. On a single-user deployment this should be near zero. A consistently high queue wait indicates multiple users are querying simultaneously and the server cannot process them fast enough in parallel. Reducing this requires either concurrency optimisation or additional capacity.",
                  ms: kpi.avgQueueMs,
                  color: '#64748b',
                },
                {
                  ...
                  tooltip:
                    'Remaining response time not attributed to AI model calls, LEX API lookups, or queue wait. Covers: parsing the HTTP request and conversation history; serialising tool calls and results between agent steps; writing timing metrics and conversation logs to PostgreSQL; streaming the response to the browser; and other FastAPI/Python execution time. Under normal conditions this should be a small fraction of total time. An unexpectedly large value may indicate a slow database write, a very large payload being serialised, or a bottleneck in the Python request pipeline.',
                  ms: Math.max(0, kpi.avgTotalMs - kpi.avgLlmMs - kpi.avgLexMs - kpi.avgQueueMs),
                  color: PERF_COLORS.other,
                },
                ...
                return (
                  <div key={label}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="font-medium flex items-center" style={{ color }}>
                        {label}
              ...
              <thead>
                <tr>
                  {[
                    {
                      label: 'Request ID',
                      tip: 'Unique identifier assigned to this query when it arrived at the server. This ID is written to the uvicorn server logs alongside all timing, prompt, and error information. Search for it in the logs to retrieve a full trace: the exact prompts sent to the AI model, each LEX API request and response, and any errors that occurred.',
                    },
                    {
                      label: 'Timestamp',
                    ...
                    {
                      label: 'DB Lookup Time',
                      tip: 'Total time spent waiting for LEX API HTTP responses during this query, summed across all lookups. If this is a large share of Total Duration, the legal database was the bottleneck for this specific query. On the air-gapped internal network, individual lookups typically take under 1 second — values significantly above that warrant checking LEX server health or network conditions at the time shown in the Timestamp column.',
                    },
                    {
                      label: 'Time to First Token',
                      tip: "Time from when this query's prompt was sent to the AI model until the model produced its very first output token. Before any output appears the model must load the full context window (conversation history, system prompt, tool results) and begin generation. A high value relative to the average TTFT shown in the KPI cards above typically means this query had an unusually large input context, or the model was concurrently serving another request and this one waited.",
                    },
                  ].map(({ label, tip }) => (
                    <th
                      key={label}
              ...
              <tbody>
                {slowest.map(row => (
                  <tr key={row.requestId} className="border-b border-ink-100">
                    ...
                    <td className="px-3 py-3 font-bold text-red-600 dark:text-red-400 whitespace-nowrap">
                      {fmtMs(row.totalMs)}
                    </td>
                    <td className="px-3 py-3">{row.llmCalls}</td>
                    <td className="px-3 py-3 text-indigo-600 dark:text-indigo-400 whitespace-nowrap">
                      {fmtMs(row.llmMs)}
                    </td>
                    ...
                    <td className="px-3 py-3 text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                      {fmtMs(row.ttftMs)}
                    </td>
                  </tr>
                ))}
