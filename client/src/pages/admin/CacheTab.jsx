...
export const CacheTab = ({ cacheStats, cacheTimeframe, setCacheTimeframe }) => {
  ...
  const hasData = kpi.memoHits > 0 || kpi.cachedPromptTokens > 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-paper p-4 rounded-lg shadow">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <FlagBadge label="Tool memo" on={flags.tool_memo_enabled} />
        </div>
      ...
      {!hasData && (
        <div className="bg-paper p-8 rounded-lg shadow text-center text-ink-400 text-sm">
          No cache activity for this period. Tool-memo hits come from multi-step Deep Research runs;
          cached prompt tokens are only reported for Anthropic models on OpenRouter.
          {(!flags.prompt_caching_enabled || !flags.tool_memo_enabled) && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-paper p-4 rounded-lg shadow">
          <h3 className="text-ink-500 text-xs font-bold uppercase">Tool-Memo Hits</h3>
          <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{kpi.memoHits}</p>
          <p className="text-xs text-ink-400 mt-1">
            {kpi.memoHitRequests} of {kpi.deepResearchRequests} Deep Research requests
          </p>
        </div>
        <div className="bg-paper p-4 rounded-lg shadow">
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-paper p-6 rounded-lg shadow">
              <h2 className="text-sm font-bold mb-4">Daily Tool-Memo Hits ({timeframeLabel})</h2>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={v => [v, 'Memo hits']}
                    />
                    <Bar dataKey="memoHits" name="Memo hits" fill={CACHE_COLORS.memo} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="bg-paper p-6 rounded-lg shadow">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs">
                <thead>
                  <tr>
                    {['Timestamp', 'Request ID', 'Chat Mode', 'Memo Hits', 'Cached Tokens', 'Discount', 'Request Cost'].map(
                      label => (
                        <th
                          key={label}
