...
export default function SystemChat() {
  ...
  useEffect(() => {
    getModels()
      ...
      .catch(err => console.warn('Failed to fetch model list (login required):', err));
  }, []);
          ...
          setMessages(prev => {
            ...
            if (currentTurnIndex !== -1) {
              ...
              if (event.type === 'token') {
                ...
                const startEventIndex = currentTurn.events.findIndex(
                  e => e.type === 'api_call_start' && e.id === event.id
                );
                ...
                <div className="space-y-1 pl-4 border-l-2 border-gray-800">
                  {msg.events.map((event, j) => (
                    <div key={j} className="text-sm break-words">
                      {event.type === 'token_stream' ? (
                        ...
                        <div className="bg-gray-900 border border-purple-500/30 rounded p-2 my-2">
                          <div className="flex justify-between items-center text-purple-400 text-xs font-bold mb-1">
                            <span>
                              API CALL: {event.method} {event.url}
                            </span>
                            {event.status && (
                      ...
                      ) : event.type === 'api_call_end' ? null : ( // Hidden, merged into start
