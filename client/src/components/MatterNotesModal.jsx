...
const noteComponents = {
  ...
  hr: ({ node, ...p }) => (
    <hr {...p} style={{ border: 'none', borderTop: '1px solid var(--ink-300)', margin: '6px 0' }} />
  ),
};

const Sep = () => <div style={{ width: 1, background: 'var(--ink-200)', margin: '3px 3px' }} />;
      ...
      <Modal onClose={onClose} className="w-[80vw] h-[80vh] flex flex-col font-ui">
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 0 }}>
              <button style={tabStyle(tab === TAB_NOTES)} onClick={() => setTab(TAB_NOTES)}>
                Notes
              </button>
              <button style={tabStyle(tab === TAB_BRIEF)} onClick={() => setTab(TAB_BRIEF)}>
                AI Brief
              </button>
            </div>
          </div>
          ...
          {tab === TAB_NOTES && (
            <>
              <div className="lex-scroll" style={{ flex: 1, overflowY: 'auto', padding: '12px 20px' }}>
                {loading && <div style={{ color: 'var(--ink-400)', fontSize: 13, padding: '8px 0' }}>Loading…</div>}
                  ...
                  >
                    ...
                    >
                      <div style={{ fontSize: 11, color: 'var(--ink-400)' }}>
                        {note.message_id && (
                          <span style={{ marginRight: 8, color: 'var(--cite)' }}>Pinned from thread</span>
                        )}
          ...
          {tab === TAB_BRIEF && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              <div style={{ padding: '14px 20px 10px', flexShrink: 0, borderBottom: '1px solid var(--ink-100)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[
                      { value: 'brief', label: 'Research summary' },
                      { value: 'gaps', label: 'Research gaps' },
                    ].map(opt => (
                      <button
                        key={opt.value}
                        onClick={() => {
                          setBriefMode(opt.value);
                          setBriefContent('');
                          setBriefError('');
                        }}
              ...
              <div className="lex-scroll" style={{ flex: 1, overflowY: 'auto', padding: '14px 20px' }}>
                ...
                {briefError && <div style={{ color: 'var(--danger)', fontSize: 13 }}>{briefError}</div>}
                {briefContent && (
                  <div
                    style={{
                  ...
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        a: ({ node, ...p }) => (
                          <a {...p} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)' }} />
                        ),
              ...
              {briefContent && (
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-brand text-white font-ui text-sm font-medium rounded-md px-4 py-2 hover:bg-brand-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {briefSaved ? 'Saved!' : savingBrief ? 'Saving…' : 'Save as note'}
                  </button>
                </div>
              )}
            </div>
