  ...
  "delegations": [
    {
      ...
      "halted": null,                 // v2; {reason, limit, steps, written_up (v4)} at the step cap
      "lost": null,                   // v6; {reason: "empty_completion", sources_retrieved} when the final reply was lost
      "error": null,
...
- **`raw_result` and `final_result`** hold the retrieval as returned by the external API and the text actually placed in the Worker's context respectively. Exposing both allows a defective summary to be distinguished from a defective retrieval — an attribution that post-summarisation text alone cannot support. ~~They differ whenever `summarised` is true.~~ **They also differ whenever the server appends a bracketed instruction block, which is independent of summarisation** (corrected 2026-09-15, FIX_PLAN P2.2). `final_result` is the concatenation of the (possibly summarised) retrieval and any of: the Phase-2 nudge, the citation-URL block (P1.6, summarised results only) and the `[SEARCH SCOPE …]` block (P2.2, on **every** `search_legislation` and `search_legislation_sections` result). A consumer diffing `raw_result` against `final_result` to detect summarisation loss must therefore key on the `summarised` flag, not on the texts being unequal — those two tools' results now always differ. **Also appended since FIX_PLAN P2.4 (2026-09-17):** a `[SEARCH SCOPE — not held: …]` block on any retrieval the LEX API answered with `Legislation not found` (in practice `get_legislation_text` on an id the index does not hold). That `raw_result` is the executor's `Error executing tool: …` string, not JSON, so `raw_decode` raises on it: check for that prefix first. The case-law corpus disclosure P2.4 added is on the **answer**, not on any tool result. Parse `final_result` with a JSON `raw_decode`, which stops at the end of the leading object and ignores the appended tail; a plain parse raises "Extra data" on exactly the calls that succeeded.
- **`lookup_legislation`** *(new tool, FIX_PLAN P3.7, Sept 2026; no shape change, so `schema_version` stays 5)*: an exact test of whether the index holds one instrument, named by type, year and number. Its `raw_result` is JSON with `tool: "lookup_legislation"`, `legislation_id`, `label` (e.g. `"SSI 2025/377"`) and `status`, one of `held`, `held_without_text`, `not_held`, `lookup_failed` or `invalid`. Only the first three are statements about the index. `lookup_failed` (a timeout, a 5xx, or a 404 that is not the API's own `Legislation not found`) and `invalid` (unusable arguments) say nothing either way, and must not be counted as negatives. A held record also carries `title`, `url`, `category`, `enactment_date`, a `description` capped at 600 characters, and `text_held` (`true`, `false` for a stub, `null` if the text check did not complete). Its `api_calls` are one `POST /legislation/lookup` and, when that returns 200, one `POST /legislation/section/lookup` (id suffix `-sections`), which is the text check. **Most of these records are made by code, not by the model.** `run_worker_agent` runs the lookup on every instrument the delegation's brief names by number, before the Worker's first model call, so these records come first in `tools[]` and carry the same arguments a model would send. A model repeating one is served from the memo (`memo_hit: true`). The lookup is not a discovery search: it is never `budget_blocked`, and it is not in either efficiency phase.
...
- **`budget_blocked`** indicates that the search was hard-stopped by a discovery budget and **did not run**. ~~Applies to parliamentary modes only~~ — **since FIX_PLAN P2.7 (2026-09-17) it applies to two budgets, and they count different things.** *Parliamentary modes:* three calls to the discovery tools, checked after the memo, so a memo hit is never blocked. *Legislation modes* (`legislation_only`, `legislation_and_case_law`): `search_legislation` may be called in at most **8 ReAct rounds** per worker run (any number of calls within one round), checked **before** the memo, so a memo-servable repeat is blocked too and then carries `memo_hit: false`. `search_case_law` is not budgeted. A blocked legislation call's `raw_result` and `final_result` are both the stop message: JSON with `notice`, `instruction` and `"searched": false`, and deliberately **no** `results` or `total` key, so it must not be counted as a search that matched nothing. Exclude `budget_blocked` records from any count of searches that ran. The field is unchanged in shape, so `schema_version` is unchanged. **Since FIX_PLAN P3.1 (2026-09-18) there is a third, on the same flag:** in the legislation modes, `search_legislation_sections` may be called in at most **3 ReAct rounds per `legislation_id`** per worker run (the id normalised, so a URL and a short id are one instrument), checked **before** the memo. Its stop is JSON with `notice`, `instruction`, `legislation_id` and `"searched": false`, again with no `results`. Tell it apart from a discovery stop by the tool record's `name`; both increment `request_timings.search_budget_blocked`.
...
- **`empty_completions`** *(v3)* records provider completions that returned no content **and** no tool calls. It is `[]` on a healthy request, and a non-empty list does **not** imply the request failed — `chat_loop` retries such a completion up to three times, and `retried: true` marks an attempt the retry then recovered from. The fields exist to separate four mechanisms that were previously indistinguishable in the data: the provider returned nothing (`completion_tokens` ~0); the model spent the completion on thinking tokens (`reasoning_chars` > 0); a mid-stream failure arrived as a payload the parser used to ignore (`stream_error` set, and/or `finish_reason: "error"` with the provider's own code in `native_finish_reason`); or the model chose to say nothing. A harness watching for lost answers should treat a record with `retried: false` as one — the answer for that call was empty on every attempt, and the caller fell back to labelled research output or a notice. **Since FIX_PLAN P4.10 (no shape change, schema still v6), "every attempt" can be one:** a research Worker's completion that came back empty after heavy reasoning (10,000 or more `completion_tokens`, or 20,000 or more `reasoning_chars`) is not retried, so its only record is `attempt: 1, retried: false`, and the delegation carries `lost`. **Since FIX_PLAN P4.11 (again no shape change), the same holds for a research Worker's upstream idle timeout:** a completion whose `stream_error` reports an upstream idle timeout ("Upstream idle timeout exceeded") is not retried on a Worker call either, so it too leaves a single `attempt: 1, retried: false` record. A rate limit, also a `stream_error`, is still retried, and every non-Worker call (the Manager, planner, synthesis) still retries both. Do not read `attempt < attempts_max` on a `retried: false` record as a truncated trace.
...
- **`lost`** *(v6, FIX_PLAN P4.5)* is `null` unless the Worker's final reply came back empty on every attempt of the bounded retry, in which case it is `{"reason": "empty_completion", "sources_retrieved": N}`. The attempts themselves are the top-level `empty_completions[]` records (v3), which carry no delegation id; this field is the per-delegation outcome. It is **not** a halt: `halted` stays `null`, because no limit was reached, and `halted` keeps its meaning (the step cap). `report` then opens with an agent-addressed block, `[Research Incomplete — answer lost]` … `[/Research Incomplete]`, saying the step's answer was lost and that the search record below it (the scope block, when the step searched legislation) is a record of work, not findings. Before v6 such a `report` was the scope block alone, or `""` for a case-law step, and read like a search that found nothing: a harness grading negatives should exclude a delegation with `lost` set, and on older traces treat a `report` that is empty or the scope block alone, with `halted` and `error` both null, as the same outcome. The lawyer is told in code when the lost step was a Deep Research plan step (always) or a Manager delegation that no later delegation in the same turn made good. Since v6 the stream's `tool_end` event for a worker also names the outcome in `result`: `"Step complete"`, `"Step incomplete: stopped at the step limit"` (with `" (partial findings)"` when written up) or `"Step incomplete: no reply returned"`, and `"Research …"` for the Manager's delegations; before, it said `"Step complete"` / `"Research Complete"` for every run.
- **`schema_version`** is incremented on any change to this shape and should be asserted on by consumers. **v2** (Sept 2026) adds `delegations[].halted`; **v3** (Sept 2026) adds top-level `empty_completions[]`; **v4** (Sept 2026) adds `delegations[].halted.written_up`; **v5** (Sept 2026) adds top-level `mode_change`; **v6** (Sept 2026) adds `delegations[].lost`. All are additive, so an older consumer sees an unknown key and is otherwise unaffected — and in v3's, v5's and v6's case a key that is almost always `[]` or `null`.
...
| Instrument held? (P3.7) | `json.loads(t["raw_result"])["status"]` for `t["name"] == "lookup_legislation"`; only `held` / `held_without_text` / `not_held` are findings |
