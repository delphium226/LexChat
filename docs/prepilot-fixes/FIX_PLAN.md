...
| `[x]` | **P0.3** | **Run the baseline.** ~~n=3 for the 25 FAIL sessions, n=1 for the 16 DEFECT sessions (~$50)~~ → **n=1 across all 41, then reps 2–3 on the 12 sessions the first pass did not settle** (user decision 2026-09-14). **Done: `BASELINE.md`.** **29 of 41 reproduce, 9 do not, 3 inconclusive**; $37.62 and 6.0 h for the n=1 pass on `bc8e7a4`. Every `does-not-reproduce` is in the n=3 set, because a single draw cannot establish a negative (Invariant 4). **Three findings that change later rows:** (a) B2 fails *selectively*, not closed — see the amended P1.1; (b) the halt reaches users through the **Manager** path too, and 6383 turn 1's entire answer was the raw `[Research halted: exceeded 20 tool-call steps]` string — see the amended P2.1; (c) three of the nine non-reproductions are **not** improvements (6357 swapped a halt for a filter failure, 6381 discloses honestly but retrieves worse, 6340 swapped fabrication for a truncated non-answer). **Depends on:** P0.1, P0.2 |
