...
| `[~]` | **P0.3** | **Run the baseline.** ~~n=3 for the 25 FAIL sessions, n=1 for the 16 DEFECT sessions (~$50)~~ → **n=1 across all 41 first, then reps 2–3 only where the first pass was ambiguous or disagreed with the classification** (user decision 2026-09-14; blanket n=3 now prices at ~$140, see *Replay configuration → Cost*). Replay all 41 on current branch HEAD and record, per session, whether the original failure still reproduces. **This is the row that makes every later acceptance test meaningful.** Expect some not to reproduce — three weeks of commits landed between the pre-pilot and this branch. **Acceptance:** a `BASELINE.md` in this directory recording reproduce / does-not-reproduce / inconclusive per session, the model and date, actual spend, the reason for every does-not-reproduce, and the sessions promoted to n=3 with why. **Depends on:** P0.1, P0.2 |
