...
| | Decision | Why |
|---|---|---|
| **Model** | **Pin to `google/gemini-3.1-pro-preview` via OpenRouter** for every baseline and acceptance replay. | All **176** assistant messages in the pre-pilot ran on this model — uniform, no confound. The dev box is currently set to `moonshotai/kimi-k3` with `google/gemini-3-flash-preview` for summarisation; replaying on that measures a different system. `CLAUDE.md` already records model choice as the dominant variable (~10× swing on the same infrastructure). **First session must confirm the model is still served** — it is a preview model and may have been withdrawn. If it has, stop and re-decide; do not silently substitute. |
| **Repetitions** | **n=3 for the 25 FAIL sessions, n=1 for the 16 DEFECT sessions.** | ~$50 per pass rather than ~$82. The FAILs are where stochastic variation decides the verdict; most DEFECTs are deterministic symptoms (link format, halt text, missing subsections) that one run settles. Where a DEFECT replay is ambiguous, promote it to n=3 and note it. |
| **Cost** | ~**$27** per single full pass at pre-pilot rates; ~**$50** for the n=3/n=1 baseline; roughly **$100** including the Wave 1 re-baseline (P1.5). | Derived from actual recorded spend on those 41 sessions. Session 6406 alone cost $4.86. Record actual spend per pass in `SESSION_LOG.md` so the estimate stays honest. |
| **Credentials** | OpenRouter key is in the **local DB**, not `.env` — `app_settings` row `provider.openrouter`. `active_provider` is `openrouter`. | Settings are DB-first by design; `.env` holds only startup fallbacks. Change the model via Admin Portal → Developer, or write the `provider.openrouter` row directly. **Restore the dev box's previous model afterwards** so unrelated work is not silently running on the pinned model. |
| **Auth** | `/api/system/chat` requires `get_current_user` (`routers/system.py:76`). | The July 2026 auth hardening locked every endpoint; there is no global middleware, so the runner must log in via `/api/auth/login` and carry the cookie. |
...
| | ID | Row |
|---|---|---|
| `[ ]` | **P0.1** | **Build the replay runner.** A script that takes a session's user turns in order and replays them through `/api/system/chat` with the session's recorded filters, capturing the `audit` event and the final answer to a JSON file per run. **Files:** new `server_py/tools/replay.py` (+ `evidence/replay/` gitignored). Must log in via `/api/auth/login` and carry the cookie — the endpoint is behind `get_current_user`. **Before the first run, confirm `google/gemini-3.1-pro-preview` is still served by OpenRouter and pin it** (see *Replay configuration*); if it is not, stop and re-decide rather than substituting. **Acceptance** *(deterministic)*: replaying session 6404 (a clean PASS) produces a trace containing at least one `search_legislation` call and a non-empty answer, recorded as having run on the pinned model. **Depends on:** — |
...
| `[ ]` | **P0.3** | **Run the baseline.** **n=3 for the 25 FAIL sessions, n=1 for the 16 DEFECT sessions** (~$50; see *Replay configuration*). Replay all 41 on current branch HEAD and record, per session, whether the original failure still reproduces. **This is the row that makes every later acceptance test meaningful.** Expect some not to reproduce — three weeks of commits landed between the pre-pilot and this branch. Promote any ambiguous DEFECT to n=3 and say so. **Acceptance:** a `BASELINE.md` in this directory recording reproduce / does-not-reproduce / inconclusive per session, the model and date, actual spend, and the reason for every does-not-reproduce. **Depends on:** P0.1, P0.2 |
